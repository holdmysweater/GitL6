...
class UserLogic
{
    ...
    ) : ?array
    {
        ...
        if (Authorization::isAuthorized()) {
            $errors['others'] = 'Вы уже авторизованы (перезагрузите страницу)';
            return $errors;
        }
        ...
        if (!$user) {
            $errors['others'] = 'Ошибка добавления пользователя (обратитесь в поддержку)';
            return $errors;
        }
    ...
    public static function logIn(string $email, string $password) : ?string
    {
        ...
        if (!password_verify($password, $user_data['password'])) {
            ...
            return $max_attempts - $attempts > 0
                ...
                : 'Неверно указан пароль.' . '<br>' . 'Попробуйте еще раз через ' .  SafeString::secondsToMinutesAndSeconds(LoginAttemptsLogic::timeUntilNextAllowedAttempt($user_id));
        }
